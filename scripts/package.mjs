...
function writeOfflineGuide() {
  ...
  writeFileSync(
    ...
    [
      ...
      "2. Double-click start-modusignal.bat (Windows, needs Python 3) or run start-modusignal.sh.",
      "   Or manually: python -m http.server 4173",
      "3. Open http://localhost:4173 in Chrome or Edge.",
      "",
      "Web Serial and WebUSB require Chrome/Edge and localhost or HTTPS.",
      "",
      "modusignal 离线包",
      "",
      "1. 解压到任意目录。",
      "2. 双击 start-modusignal.bat（Windows，需已安装 Python 3）或运行 start-modusignal.sh。",
      "   也可手动执行：python -m http.server 4173",
      "3. 用 Chrome / Edge 打开 http://localhost:4173。",
      "",
      "串口和 WebUSB 功能需要 Chrome/Edge，且需在 localhost 或 HTTPS 下使用。",
      "",
    ].join("\n"),
    "utf8",
  );
